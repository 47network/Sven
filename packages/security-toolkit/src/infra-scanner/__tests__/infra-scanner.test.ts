...
describe('Infra Scanner', () => {
  describe('auditDockerCompose', () => {
    ...
    it('ignores non-dangerous capabilities', () => {
      const services: DockerComposeService[] = [
        { name: 'safe-cap-svc', cap_add: ['CHOWN', 'SETUID'] },
      ];
      const findings = auditDockerCompose(services);
      const capFindings = findings.filter((f) => f.title.startsWith('Dangerous capability added'));
      expect(capFindings).toHaveLength(0);
    });
    ...
    it('ignores safe port exposure', () => {
      const services: DockerComposeService[] = [
        { name: 'safe-port-svc', ports: ['127.0.0.1:8080:80', '[::1]:8080:80'] },
      ];
      const findings = auditDockerCompose(services);
      const widePortFinding = findings.find((f) => f.title.includes('exposed on all interfaces'));
      expect(widePortFinding).toBeUndefined();
    });
    ...
    it('ignores secure service configuration', () => {
      const services: DockerComposeService[] = [
        {
          name: 'secure-svc',
      ...
      const findings = auditDockerCompose(services);
      expect(findings.find((f) => f.title === 'No healthcheck defined')).toBeUndefined();
      expect(findings.find((f) => f.title === 'Container filesystem is writable')).toBeUndefined();
      expect(findings.find((f) => f.title === 'Container may run as root')).toBeUndefined();
    });
  });
  ...
  describe('auditEnvFile', () => {
    ...
    it('ignores empty lines, comments, and invalid formats', () => {
      const content = '\n# A comment\nINVALID_FORMAT\n';
      const findings = auditEnvFile(content, '.env');
      expect(findings).toHaveLength(0);
    });

    it('ignores non-development values for debug keys', () => {
      const content = ['DEBUG=false', 'NODE_ENV=production', 'ENVIRONMENT=prod'].join('\n');
      const findings = auditEnvFile(content, '.env.production');
      expect(findings).toHaveLength(0);
    });
  });
  ...
  describe('generateInfraReport', () => {
    ...
    it('clamps negative scores to zero', () => {
      const findings: InfraFinding[] = [
        { id: '1', category: 'docker-compose', severity: 'critical', title: 'T1', description: 'D1', location: 'L1', remediation: 'R1' },
        { id: '2', category: 'docker-compose', severity: 'critical', title: 'T2', description: 'D2', location: 'L2', remediation: 'R2' },
        { id: '3', category: 'docker-compose', severity: 'critical', title: 'T3', description: 'D3', location: 'L3', remediation: 'R3' },
        { id: '4', category: 'docker-compose', severity: 'critical', title: 'T4', description: 'D4', location: 'L4', remediation: 'R4' },
        { id: '5', category: 'docker-compose', severity: 'critical', title: 'T5', description: 'D5', location: 'L5', remediation: 'R5' },
        { id: '6', category: 'docker-compose', severity: 'critical', title: 'T6', description: 'D6', location: 'L6', remediation: 'R6' },
      ];
      const report = generateInfraReport(findings);
