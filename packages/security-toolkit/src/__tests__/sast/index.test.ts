...
describe('SAST Module', () => {
  describe('scanSource', () => {
    ...
    it('should detect SQL injection vulnerability', () => {
      ...
      expect(sqlFinding?.severity).toBe('critical'); // SAST-001 is critical
  ...
  describe('scanFiles', () => {
    it('should scan multiple files and aggregate findings correctly', () => {
      const files = new Map<string, string>([
        ...
        ['vuln1.js', `db.query("SELECT * FROM t WHERE id=" + id);`], // SAST-001 (critical)
        ['vuln2.js', `fetch(req.query.data);`] // SAST-003 (high)
      ]);
    ...
    it('should correctly calculate security score', () => {
       const files = new Map<string, string>([
        ['vuln.js', `db.query("SELECT * FROM t WHERE id=" + id);`] // SAST-001: critical (penalty: 20)
      ]);
      const report = scanFiles(files);
      expect(report.securityScore).toBe(80); // 100 - 20
    });
  });
  ...
  describe('Utility functions', () => {
    ...
    it('filterRules should correctly filter by severity and category', () => {
      ...
      const specificRules = filterRules({ severity: 'critical', category: 'sql-injection' }); // Fixed severity to match SAST-001
