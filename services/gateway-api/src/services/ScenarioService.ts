...
export async function getScenarioVariations(
  ...
  try {
    ...
    const query = `SELECT v.id, v.scenario_id, v.name, v.parameters, v.override_message,
    ...
    const finalQuery = `${query}${orgClause} ORDER BY v.created_at`;
    const result = await pool.query(finalQuery, params);
...
export async function getScenarioCategories(organizationId?: string): Promise<string[]> {
  try {
    const query = `SELECT DISTINCT category FROM scenarios WHERE category IS NOT NULL`;
    const params: any[] = [];
    const orgClause = organizationId ? ' AND organization_id = $1' : '';
    if (organizationId) {
      params.push(organizationId);
    }
    const finalQuery = `${query}${orgClause} ORDER BY category`;
    const result = await pool.query(finalQuery, params);
