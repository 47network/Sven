...
const RUNBOOK = path.resolve(__dirname, '../../../../deploy/multi-server/RUNBOOK.md');
const RESTART_HEALTH_CHECK = path.resolve(__dirname, '../../../../scripts/multi-server-restart-health-check.cjs');
...
describe('vm restart drill contract', () => {
  it('exposes npm scripts for restart health and VM restart drill checks', async () => {
    ...
    expect(scripts['release:multi-server:restart:health:check']).toBe('node scripts/multi-server-restart-health-check.cjs --strict');
  ...
  it('keeps restart health checker pinned to VM5 and VM7 compose coverage', async () => {
    ...
    expect(source).toContain('deploy/multi-server/docker-compose.vm5-ai.yml');
    expect(source).toContain('deploy/multi-server/docker-compose.vm7-adapters.yml');
    ...
    expect(source).toContain('multi-server-restart-health-latest.json');
  });
  ...
  it('keeps VM restart drill script pinned to waited docker compose commands and evidence outputs', async () => {
    ...
    expect(source).toContain('release:multi-server:restart:health:check');
  ...
  it('documents planned and executable VM restart drill commands in the multi-server runbook', async () => {
