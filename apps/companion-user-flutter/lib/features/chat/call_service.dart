...
class CallService {
  ...
  Future<CallStartResult?> startCall(String chatId, {String type = 'voice'}) async {
    ...
    if (response.statusCode == 409) {
      ...
      final data = jsonDecode(response.body)['data'] as Map<String, dynamic>?;
      if (data != null) return CallStartResult(callId: data['call_id'] as String, iceServers: []);
    }
    return null;
  }
  ...
  Future<List<ActiveCall>> getActiveCalls(String chatId) async {
    ...
    return calls.map((c) => ActiveCall.fromJson(c as Map<String, dynamic>)).toList();
  }
  ...
  Future<List<IceServer>> getIceConfig() async {
    ...
    return servers.map((s) => IceServer.fromJson(s as Map<String, dynamic>)).toList();
  }
}
...
class CallStartResult {
  const CallStartResult({required this.callId, required this.iceServers, this.callType, this.status});
  final String callId;
  final List<IceServer> iceServers;
  final String? callType;
  final String? status;

  factory CallStartResult.fromJson(Map<String, dynamic> json) => CallStartResult(
...
class CallJoinResult {
  const CallJoinResult({required this.callId, required this.participants, required this.iceServers});
  final String callId;
...
class CallParticipant {
  const CallParticipant({required this.userId, required this.status, this.displayName, this.mediaState});
  final String userId;
  final String status;
...
class ActiveCall {
  const ActiveCall({required this.id, required this.callType, required this.status, this.participantCount = 0});
  final String id;
  final String callType;
  final String status;
