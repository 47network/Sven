...
class _MediaGalleryPageState extends State<MediaGalleryPage>
    ...
    if (items.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.photo_library_outlined, size: 48, color: tok.onSurface.withValues(alpha: 0.15)),
            const SizedBox(height: 8),
            Text('No media yet', style: TextStyle(color: tok.onSurface.withValues(alpha: 0.4))),
          ],
        ),
      );
    }
                ...
                if (isImage)
                  Image.network(
                    ...
                    errorBuilder: (_, __, ___) => Container(
                      color: tok.card,
                      child: Icon(Icons.broken_image, color: tok.onSurface.withValues(alpha: 0.3)),
                    ),
                  )
                else
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          item.fileName,
                          style: TextStyle(fontSize: 10, color: tok.onSurface.withValues(alpha: 0.6)),
                          maxLines: 1,
    ...
    if (items.isEmpty) {
      return Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.folder_open, size: 48, color: tok.onSurface.withValues(alpha: 0.15)),
            const SizedBox(height: 8),
            Text('No files yet', style: TextStyle(color: tok.onSurface.withValues(alpha: 0.4))),
          ],
        ),
      );
    }
        ...
        return Card(
          ...
          child: ListTile(
            leading: CircleAvatar(
              ...
              child: Icon(_fileIcon(item.mimeType), color: tok.secondary, size: 22),
            ),
            title: Text(
              item.fileName,
            ...
            subtitle: Text(
              ...
              style: TextStyle(fontSize: 12, color: tok.onSurface.withValues(alpha: 0.5)),
            ),
  ...
  void _showPreview(BuildContext context, MediaUpload item, SvenModeTokens tok) {
    final base = ApiBaseService.currentSync();
    showDialog(
      context: context,
      builder: (ctx) => Dialog(
        ...
        child: ClipRRect(
          ...
          child: Image.network(
            ...
            errorBuilder: (_, __, ___) => Container(
              ...
              color: tok.card,
              child: Center(
                child: Icon(Icons.broken_image, color: tok.onSurface.withValues(alpha: 0.3), size: 48),
              ),
            ),
          ),
        ),
