...
interface Props {
  ...
  onSelect?: (parcel: EidolonParcel) => void;
}

export function ParcelGrid({ parcels, selectedId, onSelect }: Props) {
  ...
  const handleClick = useCallback(
    (parcel: EidolonParcel, e: { stopPropagation: () => void }) => {
      e.stopPropagation();
      onSelect?.(parcel);
    },
    [onSelect],
      ...
      {parcelMeshes.map((p) => {
        ...
        return (
          <group key={p.id} position={[p.gridX, 0, p.gridZ]}>
            {/* Parcel ground plate — clickable when onSelect is wired */}
            <mesh
              position={[0, 0.05, 0]}
              receiveShadow
              onClick={onSelect ? (e) => handleClick(p, e) : undefined}
            ...
            {isSelected && (
              <mesh position={[0, 0.12, 0]} rotation={[-Math.PI / 2, 0, 0]}>
                <ringGeometry args={[p.scale * 0.55, p.scale * 0.6, 32]} />
