## eidolon: treasury panel, event toasts, interactive minimap, keyboard cycling, scene error boundary
- `apps/eidolon-ui/src/components/TreasuryPanel.tsx` — **NEW**: at-a-glance treasury dashboard. Shows `totalBalanceUsd` headline, horizontal bar chart breakdown by kind (listing_revenue, task_revenue, subscription, refund, credit, debit, settlement), amber `openApprovals` badge when > 0, last settlement timestamp in Bucharest time. Sorted by absolute value; colour-coded per kind.
- `apps/eidolon-ui/src/hooks/useEventToasts.ts` — **NEW**: hook watching SSE events for high-impact kinds (anomaly, oversight, business_created, spawned, retired, book_published, goal_completed). Returns `{ toasts, dismiss }`. Deduplicates by event id, auto-dismisses after 6 s, caps at 4 simultaneous toasts. Seen-set capped at 200 entries to prevent memory growth.
- `apps/eidolon-ui/src/components/ToastStack.tsx` — **NEW**: renders auto-dismissing notification cards in top-right. Colour-coded borders per event kind (rose for anomaly, amber for oversight, emerald for business, etc.). Slide-in animation, dismiss button, `role="log" aria-live="polite"` for screen-reader accessibility.
- `apps/eidolon-ui/src/components/SceneErrorBoundary.tsx` — **NEW**: React class error boundary wrapping the R3F canvas. If the 3D scene throws, renders a compact error card with the message and a retry button. Rest of HUD stays fully functional.
- `apps/eidolon-ui/src/components/MiniMap.tsx` — **interactive**: removed `pointer-events-none`, added click handlers on building circles, citizen circles, and parcel rects. New props: `onSelectBuilding`, `onSelectCitizen`, `onSelectParcel`. Hit targets enlarged for usability. Cursor pointer and hover opacity boost on all clickable elements.
- `apps/eidolon-ui/src/app/page.tsx` — wired TreasuryPanel in left sidebar, ToastStack after EventFeed, SceneErrorBoundary around CityScene, MiniMap callbacks (onSelectBuilding/onSelectCitizen/onSelectParcel). Added keyboard cycling: ↑/↓ arrow keys cycle through citizens with runtime state (wraps around), Esc clears selection. Input fields excluded from capture.
- `apps/eidolon-ui/src/app/globals.css` — added `@keyframes slide-in-right` and `.animate-slide-in-right` utility class for toast entrance animation.

## eidolon: camera fly-to, citizen tooltip, SSE indicator, new-events badge, day/night atmosphere
- `apps/eidolon-ui/src/components/CityScene.tsx` — **Camera fly-to**: new `CameraFlyTo` inner component lerps OrbitControls target + camera position toward the selected entity using `useFrame`; stops when distance < 0.3. `OrbitControls` now has `makeDefault` so `useThree().controls` returns the instance. Added `focusPosition` prop (`{ x, z } | null`). **Day/night atmosphere**: fog colour, fog distance, and `<Stars>` factor now respond to `worldTime.phase` — night is deepest black with close fog and bright stars; dawn/dusk get transitional tints; day opens fog distance and dims stars.
- `apps/eidolon-ui/src/components/Citizen.tsx` — **Hover tooltip**: on `pointerOver`, renders a `<Html>` tooltip (drei) showing citizen name (with mood emoji), state label, and energy %. Only shows when not selected (avoids clutter with the InspectorPanel). `distanceFactor={60}`, glass-card styling, pointer-events-none.
- `apps/eidolon-ui/src/hooks/useEidolonEvents.ts` — **SSE status tracking**: hook now returns `{ events, sseStatus }` instead of a flat array. New exported `SseStatus` type (`'connecting' | 'connected' | 'reconnecting'`). Sets status on `onopen` (→ connected), `onerror` (→ reconnecting), and initial connect (→ connecting/reconnecting).
- `apps/eidolon-ui/src/app/page.tsx` — Destructured `{ events, sseStatus }` from hook. Added `SseIndicator` component in header: green dot when connected, amber pulsing dot + label when reconnecting, gray pulsing dot when connecting. Added `focusPosition` memo (resolves selected citizen/building/parcel → `{ x, z }`). Wired `focusPosition` to CityScene.
- `apps/eidolon-ui/src/components/EventFeed.tsx` — **New-events badge**: scroll tracking via `useRef` + `onScroll`. When the user has scrolled past 12px from top, new events increment a badge counter. "↑ N new events" button appears above the list; clicking scrolls to top and resets count. Added `useCallback`, `useEffect`, `useRef` imports.
- Verified on VM4 (10.47.47.8): build exit 0, rsync `.next/`, `pm2 restart sven-eidolon-ui` clean. App ready on port 3311.

## eidolon: clickable citizens, EventFeed filters + navigation, SSE full coverage, MiniMap citizen highlight
- `apps/eidolon-ui/src/components/Citizen.tsx` — citizens are now clickable in the 3D scene. Added `selected` and `onSelect` props. Click handler with `stopPropagation`, pointer cursor on hover. Selected citizen gets a cyan torus selection ring that orbits at 1.5 rad/s via `useFrame`. Mesh wrapped in `<group>` for proper event handling.
- `apps/eidolon-ui/src/components/CityScene.tsx` — added `selectedCitizenId` / `onSelectCitizen` props, wired through `CityContent` to each `<Citizen>`. Selection state compared via `selectedCitizenId === c.id`.
- `apps/eidolon-ui/src/components/EventFeed.tsx` — complete rewrite with two new features: **Category filter tabs** — 8 categories (All, Market, Agent, Treasury, World, Crew, Publish, Other) with colour-coded toggle buttons. Events grouped by prefix into categories; "Other" catches uncategorised kinds. Active tab gets a white background highlight. **Click-to-navigate** — events with `agentId`/`buildingId` in payload are clickable. Clicking navigates to the entity in the inspector. Arrow indicator (↗) shown on navigable events. Full keyboard accessibility (Enter/Space). New props: `onSelectCitizen`, `onSelectBuilding`.
- `apps/eidolon-ui/src/hooks/useEidolonEvents.ts` — expanded named SSE event listeners from 10 to all 35 event kinds. Previously only market/treasury/agent.spawned/agent.retired/infra/heartbeat were registered; now all agent, publishing, crew, goal, world, and oversight events have dedicated listeners. Prevents silent event drops when the backend sends named SSE events.
- `apps/eidolon-ui/src/components/MiniMap.tsx` — added `selectedCitizenId` prop. Selected citizen dot is now highlighted: larger radius (3 vs 1), cyan fill, full opacity, white stroke. Unselected dots remain small/translucent white.
- `apps/eidolon-ui/src/app/page.tsx` — wired all new props: `selectedCitizenId` + `onSelectCitizen` to CityScene, `onSelectCitizen` + `onSelectBuilding` to EventFeed, `selectedCitizenId` to MiniMap.
- Verified on VM4 (10.47.47.8): build exit 0, rsync `.next/`, `pm2 restart sven-eidolon-ui` clean. App ready on port 3311.

## eidolon: EventFeed full accents, MiniMap, staleness badge, parcel owner label fix
- `apps/eidolon-ui/src/app/page.tsx` — fixed `c?.name` → `c?.label` in `parcelOwnerLabel` memo (EidolonCitizen has `label`, not `name` — owner name was always falling back to raw agentId). Added `lastFetchRef` to track last successful fetch timestamp. Added inline `StaleBadge` component: updates every 1s, shows data age in seconds, goes amber >30s and red >60s — alerts operators to stale data. Wired `StaleBadge` next to `WorldTimeBadge` in header. Added `MiniMap` import and rendered it with buildings/citizens/parcels/selection state.
- `apps/eidolon-ui/src/components/EventFeed.tsx` — expanded `KIND_ACCENT` from 8 entries to 35 covering all event kinds. Changed type from `Record` to `Partial<Record>` since the full union is large. Events that were previously all gray now have semantically appropriate accent colours (market=emerald/sky, treasury=amber/rose, agent=violet/sky/amber, publishing=violet/pink, crew=pink, goal=teal/cyan, oversight=rose, world=gray/cyan, infra=sky).
- `apps/eidolon-ui/src/components/MiniMap.tsx` — new component. 140×140px SVG overlay in bottom-right corner. Shows all buildings as coloured dots (by kind), citizens as tiny white dots, parcels as small squares. Selected building/parcel highlighted with full opacity + white stroke. Crosshair grid lines for spatial orientation. `pointer-events-none` so it doesn't block 3D scene interaction. World space ±200 mapped to pixel space.
- Verified on VM4 (10.47.47.8): build exit 0, rsync `.next/`, `pm2 restart sven-eidolon-ui` clean. HTTP 200 on 3311. PM2 logs clean.

## eidolon: clickable parcels, ParcelInspector, citizen↔parcel cross-links, event glow coverage
- `apps/eidolon-ui/src/components/ParcelGrid.tsx` — parcels are now clickable. Added `selectedId` / `onSelect` props. Ground plates respond to click (with `stopPropagation`), show pointer cursor on hover. Selected parcel gets a white selection ring (zone-coloured `ringGeometry`), brighter opacity (0.7 vs 0.4), and enhanced emissive on structures (0.6 vs 0.3).
- `apps/eidolon-ui/src/components/CityScene.tsx` — added `selectedParcelId` / `onSelectParcel` props, wired through `CityContent` to `ParcelGrid`.
- `apps/eidolon-ui/src/components/InspectorPanel.tsx` — new `ParcelInspector` component: zone badge, size header, currentLocation chip, ✕ close, owner cross-link button (resolves agentId → citizen name), stats grid (land value, invested, city visits, last visit, acquired date, grid coords), structures list (type/label/level), parcel ID footer. New `ParcelStat` sub-component in `CitizenInspector`: clickable cross-link jumping from citizen to their parcel (shows zone + size). Precedence order: citizen > parcel > building.
- `apps/eidolon-ui/src/app/page.tsx` — added `selectedParcel` state with mutual exclusion (selecting parcel clears building + citizen and vice versa). `parcelOwnerLabel` memo resolves parcel's agentId to citizen display name. `citizenParcel` memo resolves selected citizen's parcel. Cross-link callbacks: `handleSelectParcelOwner` (parcel→citizen), `handleSelectParcelByAgentId` (citizen→parcel). All props wired to CityScene and InspectorPanel.
- `apps/eidolon-ui/src/hooks/useEventGlow.ts` — added 10 new event→glow mappings: `agent.anomaly_detected`, `agent.report_generated`, `oversight.command_issued`, `agent.moved`, `agent.built_structure`, `agent.parcel_acquired`, `agent.message_sent`, `goal.progress`, `publishing.review_submitted`, `world.parcel_interaction`. Total EVENT_TO_KIND coverage: 31 events.
- Verified on VM4 (10.47.47.8): build exit 0, rsync `.next/`, `pm2 reload sven-eidolon-ui` clean. HTTP 200 on 3311. PM2 logs clean — zero errors post-reload.

## eidolon: cross-link inspector home↔occupants, Esc-to-deselect, citizen search, SSR fix
- `apps/eidolon-ui/src/app/page.tsx` — rebuilt from clean pre-Batch-B baseline (prior version was corrupted on disk from a bad multi_replace) and extended with Batch C features. Adds `handleClearAll` callback + global `Escape` keyboard listener that clears both `selected` (building) and `selectedCitizenId` — operators can now dismiss the inspector without clicking ✕. Adds `buildingOccupants` (citizens whose `homeBuildingId` matches the selected building, capped at 12). Adds `handleSelectBuildingById` (resolves building by id for citizen "Home" cross-link). Passes new props to `InspectorPanel`: `buildingOccupants`, `onSelectBuildingById`, `onSelectCitizen`, `onClose`. `WorldTimeBadge` changed from static import to `dynamic(... { ssr: false })` — fixes recurring `ReferenceError: worldTime is not defined` in SSR (digest `2408385410`).
- `apps/eidolon-ui/src/components/InspectorPanel.tsx` — extended `Props` with `buildingOccupants`, `onSelectBuildingById`, `onSelectCitizen`. **BuildingInspector** now shows an "occupants" section with clickable citizen chips when `occupants.length > 0` — each chip calls `onSelectCitizen(citizenId)` to jump from building→citizen. Added ✕ close button to building inspector (was only on citizen inspector). **CitizenInspector** "Home" stat replaced with a `HomeStat` button: clicking jumps to the home building via `onSelectBuildingById`. Falls back to plain stat when no callback is wired.
- `apps/eidolon-ui/src/components/CitizenStripPanel.tsx` — added `useState` for a search filter query. When >5 citizens are present, renders a filter `<input>` matching against label, state, and mood (case-insensitive). Shows `filteredCount/totalCount` in the header. "no matches" placeholder when filter yields zero results. Still capped at 8 visible rows after filtering.
- Verified on VM4 (10.47.47.8): build exit 0, rsync `.next/`, `pm2 reload sven-eidolon-ui` clean. HTTP 200 on 3311. Flushed logs + 10s soak: zero `ReferenceError`, zero `TypeError` in fresh window.

## eidolon: clickable citizen inspector, world-time badge, next-tick countdown
- `apps/eidolon-ui/src/components/InspectorPanel.tsx` — rewritten as a unified inspector that handles **both buildings and citizens** (citizen takes precedence when both are set). New props: `citizen?`, `citizenRuntime?`, `onClose?`. The new `CitizenInspector` surfaces mood emoji + name + role + archetype, runtime status chip (idle/exploring/travelling/talking/working/building/returning_home/resting), a colour-graded energy bar (emerald ≥66 / amber ≥33 / rose <33), 2-col stat grid (state, heading-to, mood, earnings, home), top-6 specialisations as chips, and the citizen bio (line-clamp-3). ✕ button calls `onClose`. Existing building inspector behaviour preserved verbatim.
- `apps/eidolon-ui/src/components/CitizenStripPanel.tsx` — extended with optional `selectedId` and `onSelect` props. Rows render as `<button>` (with `aria-pressed`, focus-visible ring, hover state) when `onSelect` is provided, otherwise as static `<div>` (backward-compatible). Selected row gets `bg-brand-500/15 ring-1 ring-brand-400/40`. The original `agent:`-prefixed citizen id is preserved in row data so the page can match it cleanly against `snapshot.citizens[].id`.
- `apps/eidolon-ui/src/components/WorldTimeBadge.tsx` (new) — small read-only header badge that surfaces the simulated day/night cycle (which already drives `CityScene` lighting) alongside the operator's local Bucharest wall-clock. Shows phase emoji (🌅/☀️/🌇/🌙) + "phase · day N" + mono `HH:MM`. Tooltip exposes the in-world `HH:MM` (computed from `dayFraction*24` of the 60×-speed Bucharest-anchored world clock). Wall-clock re-renders every 30s; world phase comes from the existing `useWorldTime()` hook (no new ticking).
- `apps/eidolon-ui/src/components/WorldPulsePanel.tsx` — added a `NextTickCountdown` component next to the latest tick row. Computes mean tick interval from up to the last 6 ticks (chronological, consecutive `startedAt` deltas, finite/positive only) and shows ETA as `Ns` or `Nm Ss`, switching to amber `due now` when overdue. Falls back to `next: —` when fewer than 2 ticks. Operators can now see tick cadence drift in real time without staring at the sparkline.
- `apps/eidolon-ui/src/app/page.tsx` — added `selectedCitizenId` state alongside the existing building `selected` state. `handleSelectBuilding` and `handleSelectCitizen` callbacks are mutually exclusive (selecting one clears the other) so the single bottom-left inspector slot is never split. `WorldTimeBadge` mounts in the header next to the brand card. `CitizenStripPanel` and `InspectorPanel` receive the wired props; the `agent:` prefix is stripped when looking up `agentStates` for the citizen runtime payload.
- Verified on VM4 (10.47.47.8): build exit 0, route `/` 7.67 kB / 95 kB first-load JS. Rsynced `.next/` (93 KB delta), `pm2 reload sven-eidolon-ui` clean, new page chunk `page-ee355652c50228ec.js` deployed. HTTP 200 on 3311 in 121 ms. Two consecutive curl probes generated zero new errors in `pm2 logs --nostream`; last `ReferenceError: worldTime is not defined` was at 03:16 UTC (pre-hotfix), post-reload window starting 05:43 is clean.

## eidolon: hotfix CityScene `worldTime` ReferenceError + click-through interactions, energy/mood badges, tick sparkline
- `apps/eidolon-ui/src/components/CityScene.tsx` — **hotfix**: outer `CityScene` JSX referenced `worldTime.phase` (lighting tints) but `const worldTime = useWorldTime()` was declared inside the inner `CityContent` component, producing `ReferenceError: worldTime is not defined` (digest `2408385410`) on every SSR render in production since the previous deploy. Moved the `useWorldTime()` call into `CityScene` where it is consumed and removed it from `CityContent` (which never used it). Also removed accidentally duplicated `parcelByAgent` + `movements` `useMemo` blocks (second copy was identical to the first; SWC accepted the redeclaration silently).
- `apps/eidolon-ui/src/components/WorldPulsePanel.tsx` — recent-talk rows are now keyboard-focusable buttons that toggle expansion: opening a row reveals the full untruncated message, the ★ marker for `influencedDecision`, both participants (`agentA ↔ agentB`), and `location`. Adds an inline `TickSparkline` (pure SVG, 160×28, no deps) plotting the last ≤24 ticks of revenue (cyan) and interactions (violet) so operators can see throughput trend at a glance.
- `apps/eidolon-ui/src/components/CitizenStripPanel.tsx` (new) — compact per-citizen strip surfacing the runtime telemetry that WorldPulsePanel only exposes in aggregate. For every citizen with a row in `world.agentStates`: mood emoji (happy/neutral/tired/frustrated/excited/curious), name (from `citizens.label`, falls back to agentId), 3-segment colour-graded energy bar (green ≥66 / amber ≥33 / rose <33), short state label. Capped at 8 rows to avoid HUD overflow; renders nothing when no agentStates are present.
- `apps/eidolon-ui/src/app/page.tsx` — wires `CitizenStripPanel` into the top-left `<aside>` stack beneath `WorldPulsePanel`.
- Verified on VM4: build clean, fresh standalone rsync to `/srv/sven/prod/eidolon-ui/`, `pm2 reload sven-eidolon-ui` clean. New page chunk `page-53e1e06ef65e0728.js` deployed. HTTP 200 on 3311. `pm2 logs sven-eidolon-ui --lines 50` shows zero `worldTime is not defined` errors after the 05:35 reload (last occurrence at 03:16, pre-fix).
